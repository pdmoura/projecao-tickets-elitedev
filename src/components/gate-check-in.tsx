...
type GateCheckInProps = { events: PublishedEventSummary[] };
...
function ValidationResult({ onReset, value }: { onReset: () => void; value: CheckInResult }) {
  ...
  return (
    <section aria-live="assertive" className={`border-2 ${appearance.border} bg-gate-surface-raised p-7 sm:p-10`} role="status">
      <div aria-hidden="true" className={`flex size-20 items-center justify-center rounded-full border-2 ${appearance.border} font-code text-5xl ${appearance.text}`}>{appearance.icon}</div>
